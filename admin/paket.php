...
<div class="min-h-screen flex">
  ...
  <div class="bg-indigo-50 flex-grow py-12 px-10">
    <div class="flex justify-between">
      <div>
        <h4 class="text-sm font-bold text-indigo-600">Hi, Your package list is here !!</h4>
        <h1 class="text-4xl font-bold text-indigo-900 mt-">--- DATA PAKET ---</h1>
      </div>
      <div>
 ...
 <!-- Componen -->
 <br></br>
    <a href = "tambah_paket.php" class = "p-2 pl-5 pr-5 bg-green-500 text-gray-100 text-lg rounded-lg focus:border-4 border-green-300">Tambahkan Paket</a>
    <div class="flex mt-6 justify-center">
    <table class="min-w-full table-auto">
    <thead class="justify-between">
              <tr class="text-center bg-indigo-600">
                <th class="px-10 py-2">
                  <span class="text-indigo-50">ID. PAKET</span>
                </th>
                <th class="px-10 py-2">
                ...
                <th class="px-10 py-2">
                  <span class="text-indigo-50">PROSES PENGERJAAN</span>
                </th>
                <th class="px-10 py-2">
    ...
    <tbody>
      ...
      if (isset($_POST["cari"])) {
          ...
          $cari = $_POST['cari'];
          $qry_paket = mysqli_query($koneksi, "select * from paket where id_paket='$cari' or jenis_paket like'%$cari%'");
      }
      else {
      ...
      ?>
        <tr class="bg-white lg:hover:bg-gray-100 flex lg:table-row flex-row lg:flex-row flex-wrap lg:flex-no-wrap mb-10 lg:mb-0">
            ...
            <td class="w-full lg:w-auto p-3 text-gray-800 text-center border border-b text-center block lg:table-cell relative lg:static">
            <?php echo $data_paket["proses"]; ?>
            </td>
            <td class="w-full lg:w-auto p-3 text-gray-800 text-center border border-b text-center block lg:table-cell relative lg:static">
                <span class="lg:hidden absolute top-0 left-0 bg-blue-200 px-2 py-1 text-xs font-bold uppercase">Actions</span>
                <a href="ubah_paket.php?id_paket=<?=$data_paket['id_paket']?>" class="rounded bg-green-400 py-1 px-3 text-xs font-bold text-white-600">EDIT</a>
                <a href="hapus_paket.php?id_paket=<?=$data_paket['id_paket']?>" onclick="return confirm('Apakah yakin ingin menghapus data ini?')" class="rounded bg-red-400 py-1 px-3 text-xs font-bold text-white-600 text-white-400">HAPUS</a>
            </td>
        </tr>
        <?php
        }
